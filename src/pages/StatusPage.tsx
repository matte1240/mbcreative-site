  ...
  return (
    <div className="page-container">
      <section data-reveal style={{ padding: '64px 0 0', animation: 'noc-rise .6s ease both' }}>
        <span style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--color-accent-300)' }}>Status</span>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, alignItems: 'flex-end', justifyContent: 'space-between', marginTop: 14 }}>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)', textAlign: 'right' }}>
      ...
      <section data-reveal style={{ padding: '34px 0 0', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,210px),1fr))', gap: 14 }}>
        {gauges.map((g) => (
          <div key={g.label} style={{ borderRadius: 'var(--radius-md)', background: 'var(--color-surface)', boxShadow: 'var(--shadow-sm)', padding: 15 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.09em', textTransform: 'uppercase', color: 'var(--dim)' }}>{g.label}</span>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)' }}>{g.sub}</span>
            </div>
      ...
      <section data-reveal style={{ padding: '44px 0 0' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 16 }}>
          <span style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.13em', textTransform: 'uppercase', color: 'var(--color-accent-300)' }}>Sonde</span>
          <span style={{ flex: 1, height: 1, background: 'linear-gradient(to right,var(--color-divider),transparent)' }}></span>
          <span style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)' }}>90 giorni</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {monitors.map((m) => (
            <div key={m.host} className="monitor-row" style={{ display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'center', padding: '13px 0', borderBottom: '1px solid color-mix(in srgb, var(--color-text) 8%, transparent)' }}>
              <div style={{ flex: '0 0 210px', minWidth: 0 }}>
                <div style={{ fontSize: 14 }}>{m.name}</div>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)', marginTop: 2 }}>{m.host}</div>
              </div>
              ...
              <div style={{ flex: '0 0 78px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 14, color: m.upColor }}>{m.up}</div>
              <div style={{ flex: '0 0 66px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)' }}>{m.ms}</div>
            </div>
          ))}
        </div>
      </section>

      <section data-reveal style={{ padding: '52px 0 0', display: 'flex', flexWrap: 'wrap', gap: 22 }}>
        <div style={{ flex: '1 1 380px', minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 16 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.13em', textTransform: 'uppercase', color: 'var(--color-accent-300)' }}>Volumi Longhorn</span>
            <span style={{ flex: 1, height: 1, background: 'linear-gradient(to right,var(--color-divider),transparent)' }}></span>
          </div>
          {volumes.map((v) => (
            <div key={v.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: '1px solid color-mix(in srgb, var(--color-text) 8%, transparent)' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 14, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.name}</span>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)', flex: 'none' }}>{v.size}</span>
              ...
              <span style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--color-accent-300)', flex: 'none', width: 52, textAlign: 'right' }}>3/3</span>
            </div>
          ))}
        </div>
        <div style={{ flex: '1 1 320px', minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 16 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.13em', textTransform: 'uppercase', color: 'var(--color-accent-300)' }}>Registro incidenti</span>
            <span style={{ flex: 1, height: 1, background: 'linear-gradient(to right,var(--color-divider),transparent)' }}></span>
          </div>
          {incidents.map((i) => (
            <div key={i.date + i.title} style={{ display: 'flex', gap: 14, padding: '0 0 20px' }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--dim)', letterSpacing: '.05em' }}>{i.date} · {i.dur}</div>
                <div style={{ fontSize: 14, margin: '4px 0 3px' }}>{i.title}</div>
                <p style={{ margin: 0, fontSize: 14, lineHeight: 1.5, color: 'color-mix(in srgb, var(--color-text) 64%, transparent)' }}>{i.body}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <footer className="site-footer" style={{ display: 'flex', flexWrap: 'wrap', gap: 20, justifyContent: 'space-between', padding: '40px 0 44px', marginTop: 34, borderTop: '1px solid var(--color-divider)', fontSize: 14, color: 'var(--dim)' }}>
